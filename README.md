...
서지정보는 로컬 DB에 먼저 저장하고 원문 수집·요약을 독립적으로 진행합니다. 별도 동기화 작업이 서지정보와 검증된 요약만 Supabase에 반영합니다. 클라우드 용량이 부족해도 로컬 수집은 계속할 수 있습니다. 매일 06:00 KST의 `daily-fetch.yml`은 동기화 상태 확인·분류·추천·설정된 이메일 발송을 수행합니다. [로컬 수집·동기화 구조](docs/local-catalog-sync.md)를 참고하세요.
