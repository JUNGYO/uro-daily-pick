...
test("mobile detail back navigation and saved feedback", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Back to picks" })).toBeVisible();
  await page.getByRole("button", { name: "Like paper", exact: true }).filter({ visible: true }).click();
  ...
  await expect(page.getByRole("button", { name: "Back to picks" })).toHaveCount(0);
  await expect(
    page.getByRole("link", { name: "Settings", exact: true }).filter({ visible: true }),
  ).toBeVisible();
});
test("collection creation and removal persist in the simulated API", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Upcoming journal club", exact: true })).toBeVisible();
  await page.getByLabel("Find a paper to add").fill("prostate");
  await page.getByRole("button", { name: "Search papers" }).click();
  await page.getByRole("button", { name: /^Add Personalized treatment/ }).click();
  await expect(page.getByRole("button", { name: /^Remove Personalized treatment/ })).toBeVisible();
  await page.getByRole("button", { name: "Delete Upcoming journal club", exact: true }).click();
