...
export default function Login() {
  ...
  useEffect(() => {
    if (new URLSearchParams(window.location.hash.slice(1)).has("error")) {
      setError("로그인이 완료되지 않았습니다. 다시 시도해 주세요.");
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            {error && (
              <p
                role="alert"
                className="text-[0.833rem] text-danger bg-[rgba(255,59,48,0.06)] border border-[rgba(255,59,48,0.12)] rounded-lg px-3 py-2"
              >
                {error}
              </p>
            )}
            {message && (
              <p
                role="status"
                className="text-[0.833rem] text-success bg-[rgba(52,199,89,0.06)] border border-[rgba(52,199,89,0.12)] rounded-lg px-3 py-2"
              >
                {message}
              </p>
            )}
          ...
          <div className="mt-4 text-center">
            ...
            {mode === "forgot" ? (
              <button
                ...
                className="text-[0.889rem] text-accent hover:underline"
              >
                Back to sign in
              </button>
            ) : (
              <button
                ...
                className="text-[0.889rem] text-text3 hover:text-accent"
              >
                Forgot password?
              </button>
            )}
