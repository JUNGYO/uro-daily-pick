-- Institution catalog collection is durable locally; only metadata is replicated here.
BEGIN;
SET LOCAL lock_timeout='5s';

CREATE TABLE app_private.institution_catalog_reports (
 worker_id uuid PRIMARY KEY REFERENCES app_private.institution_workers(id) ON DELETE CASCADE,
 status jsonb NOT NULL,
 reported_at timestamptz NOT NULL DEFAULT now()
);
ALTER TABLE app_private.institution_catalog_reports ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON app_private.institution_catalog_reports FROM PUBLIC,anon,authenticated;

CREATE FUNCTION public.sync_institution_catalog(p_worker_id uuid,p_token text,p_papers jsonb)
...
BEGIN
 ...
 FOR item IN SELECT value FROM jsonb_array_elements(p_papers) LOOP
  ...
  IF paper_key IS NULL AND pg_catalog.pg_database_size(pg_catalog.current_database())>=budget THEN
...
CREATE FUNCTION public.report_institution_catalog(p_worker_id uuid,p_token text,p_status jsonb)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
DECLARE field text; stamp timestamptz;
BEGIN
 PERFORM app_private.require_institution_worker(p_worker_id,p_token);
 IF p_status IS NULL OR jsonb_typeof(p_status)<>'object' OR octet_length(p_status::text)>4000
   OR NOT (p_status ?& ARRAY['local_papers','synced_papers','citation_pending','local_originals',
     'local_summaries','pending_originals','pending_summaries','sync_state','last_sync_at','registry_version'])
   OR (p_status-ARRAY['local_papers','synced_papers','citation_pending','local_originals',
     'local_summaries','pending_originals','pending_summaries','sync_state','last_sync_at','registry_version'])<>'{}' THEN
  RAISE EXCEPTION 'Invalid catalog report fields' USING ERRCODE='22023';
 END IF;
 FOREACH field IN ARRAY ARRAY['local_papers','synced_papers','citation_pending','local_originals',
   'local_summaries','pending_originals','pending_summaries'] LOOP
  IF jsonb_typeof(p_status->field)<>'number' OR (p_status->>field) !~ '^[0-9]{1,10}$' OR
    (p_status->>field)::numeric>1000000000 THEN
   RAISE EXCEPTION 'Invalid catalog count' USING ERRCODE='22023';
  END IF;
 END LOOP;
 IF (p_status->>'synced_papers')::bigint>(p_status->>'local_papers')::bigint OR
   (p_status->>'synced_papers')::bigint+(p_status->>'citation_pending')::bigint<>(p_status->>'local_papers')::bigint OR
   (p_status->>'pending_originals')::bigint>(p_status->>'local_originals')::bigint OR
   (p_status->>'pending_summaries')::bigint>(p_status->>'local_papers')::bigint THEN
  RAISE EXCEPTION 'Inconsistent catalog counts' USING ERRCODE='22023';
 END IF;
 IF jsonb_typeof(p_status->'sync_state')<>'string' OR p_status->>'sync_state' NOT IN
   ('idle','syncing','capacity_blocked','offline','error') OR
   jsonb_typeof(p_status->'registry_version')<>'string' OR length(p_status->>'registry_version') NOT BETWEEN 1 AND 100 THEN
  RAISE EXCEPTION 'Invalid catalog state' USING ERRCODE='22023';
 END IF;
 IF p_status->'last_sync_at'<>'null'::jsonb THEN
  IF jsonb_typeof(p_status->'last_sync_at')<>'string' THEN
   RAISE EXCEPTION 'Invalid sync timestamp' USING ERRCODE='22023';
  END IF;
  BEGIN stamp:=(p_status->>'last_sync_at')::timestamptz;
  EXCEPTION WHEN datetime_field_overflow OR invalid_datetime_format THEN
   RAISE EXCEPTION 'Invalid sync timestamp' USING ERRCODE='22023';
  END;
  IF NOT isfinite(stamp) OR stamp>now()+interval '5 minutes' OR stamp<DATE '2000-01-01' THEN
  ...
  END IF;
 END IF;
 INSERT INTO app_private.institution_catalog_reports(worker_id,status) VALUES(p_worker_id,p_status)
 ON CONFLICT(worker_id) DO UPDATE SET status=excluded.status,reported_at=now();
END;
$$;
REVOKE ALL ON FUNCTION public.sync_institution_catalog(uuid,text,jsonb),
 public.report_institution_catalog(uuid,text,jsonb) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.sync_institution_catalog(uuid,text,jsonb),
 public.report_institution_catalog(uuid,text,jsonb) TO anon,authenticated,service_role;

ALTER FUNCTION public.catalog_backfill_status() RENAME TO catalog_backfill_status_v26;
REVOKE ALL ON FUNCTION public.catalog_backfill_status_v26() FROM PUBLIC,anon,authenticated,service_role;
CREATE FUNCTION public.catalog_backfill_status()
RETURNS jsonb LANGUAGE sql SECURITY DEFINER SET search_path='' AS $$
 SELECT public.catalog_backfill_status_v26()||jsonb_build_object('local_catalog',coalesce((
  SELECT r.status||jsonb_build_object('available',true,'reported_at',r.reported_at,
 ...
 ),jsonb_build_object('available',false,'stale',true)));
$$;
